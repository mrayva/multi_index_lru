...
// wire::Reader) is expected to propagate to server_dispatch.hpp's single
// try/catch, not be caught here: `active_request` is passed by reference,
// still bound to the router's own local, still-intact object, so the
// router's catch block can always respond with it directly as long as
// parsing (which is all that can throw) happens before any
// std::make_shared/move of `active_request` below -- exactly the same
// invariant the pre-split single function relied on.
inline void dispatch_read_request(NameCache& name_cache, IdCache& id_cache, NatsBridge& nats,
                                   const std::string& name_bucket, const std::string& id_bucket,
                                   CompletionQueue& completions, KeyOperationQueue& key_queue,
                                   std::size_t max_getall_results, wire::Op op, wire::KeyKind kind, wire::Reader& r,
